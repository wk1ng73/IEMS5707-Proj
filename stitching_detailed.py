...
parser.add_argument(
    ...
    help="Type of estimator used for transformation estimation. The default is '%s'."
...
parser.add_argument(
    ...
    action='store',
    default=0,
    help="crop out the largest rectangular region from the stitched image.",
    type=int,
    dest='crop'
)
...
def main():
    ...
    print(img_dir_name)
    img_names = sorted(list(paths.list_images(img_dir_name)))
    print(img_names)
    ...
    if not timelapse:
        ...
        dst = cv.normalize(
            src=result, dst=None, alpha=255.0, norm_type=cv.NORM_MINMAX, dtype=cv.CV_8U
        )
    ...
    with open('log.txt','a') as f:
        f.write("{}:{}ms\n".format(args.features, ((T2 - T1)*1000)))

    if args.crop > 0:
        stitched = cv.imread(result_name)
        
        # create a 10 pixel border surrounding the stitched image
        print("[INFO] cropping...")
        stitched = cv.copyMakeBorder(
            stitched, 10, 10, 10, 10, cv.BORDER_CONSTANT, (0, 0, 0)
        )
        ...
        cnts = cv.findContours(
            minRect.copy(), cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE
        )
        cnts = imutils.grab_contours(cnts)
        c = max(cnts, key=cv.contourArea)
        (x, y, w, h) = cv.boundingRect(c)
        ...
        result_crop_name = os.path.splitext(result_name)[0] + '_crop' +  os.path.splitext(result_name)[1]
        cv.imwrite(result_crop_name, stitched)
